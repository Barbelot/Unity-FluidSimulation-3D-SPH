...
namespace Seb.Helpers
{
	...
	public static class ComputeHelper
	{
		...
		public static GraphicsBuffer CreateAppendBuffer<T>(int size = 1)
		{
			int stride = GetStride<T>();
			GraphicsBuffer buffer = new GraphicsBuffer(GraphicsBuffer.Target.Append, size, stride);
			buffer.SetCounterValue(0);
			return buffer;
		}


		public static void CreateAppendBuffer<T>(ref GraphicsBuffer buffer, int count)
		{
			int stride = GetStride<T>();
			...
			{
				Release(buffer);
				buffer = new GraphicsBuffer(GraphicsBuffer.Target.Append, count, stride);
			}

			buffer.SetCounterValue(0);
		}

		public static bool CreateStructuredBuffer<T>(ref GraphicsBuffer buffer, int count)
		{
			int stride = GetStride<T>();
			bool createNewBuffer = buffer == null || !buffer.IsValid() || buffer.count != count || buffer.stride != stride;
			if (createNewBuffer)
		...
		public static void CreateStructuredBuffer<T>(ref GraphicsBuffer buffer, List<T> data) where T : struct
		{
			...
			{
				Release(buffer);
				buffer = new GraphicsBuffer(GraphicsBuffer.Target.Structured, data.Count, stride);
			}
		...
		public static void CreateStructuredBuffer<T>(ref GraphicsBuffer buffer, T[] data)
		{
			CreateStructuredBuffer<T>(ref buffer, data.Length);
			buffer.SetData(data);
		}

		public static void SetBuffer(ComputeShader compute, GraphicsBuffer buffer, string id, params int[] kernels)
		{
		...
		public static void SetBuffers(ComputeShader cs, int kernel, Dictionary<GraphicsBuffer, string> nameLookup, params GraphicsBuffer[] buffers)
		{
		...
		public static GraphicsBuffer CreateAndSetBuffer<T>(T[] data, ComputeShader cs, string nameID, int kernelIndex = 0)
		{
		...
		public static void CreateAndSetBuffer<T>(ref GraphicsBuffer buffer, T[] data, ComputeShader cs, string nameID, int kernelIndex = 0)
		{
			CreateStructuredBuffer<T>(ref buffer, data.Length);
			buffer.SetData(data);
		...
		public static GraphicsBuffer CreateAndSetBuffer<T>(int length, ComputeShader cs, string nameID, int kernelIndex = 0)
		{
			GraphicsBuffer buffer = null;
		...
		public static void CreateAndSetBuffer<T>(ref GraphicsBuffer buffer, int length, ComputeShader cs, string nameID, int kernelIndex = 0)
		{
		...
		public static GraphicsBuffer CreateArgsBuffer(Mesh mesh, int numInstances)
		{
			...
			GraphicsBuffer argsBuffer = new GraphicsBuffer(GraphicsBuffer.Target.IndirectArguments, numArgs, stride);
			argsBuffer.SetData(args);
			return argsBuffer;
		}

		public static void CreateArgsBuffer(ref GraphicsBuffer buffer, uint[] args)
		{
			const int stride = sizeof(uint);
			const int numArgs = 5;
			if (buffer == null || buffer.stride != stride || buffer.count != numArgs || !buffer.IsValid())
			{
				buffer = new GraphicsBuffer(GraphicsBuffer.Target.IndirectArguments, numArgs, stride);
			}
		...
		public static void CreateArgsBuffer(ref GraphicsBuffer buffer, GraphicsBuffer appendBuffer)
		{
			const int stride = sizeof(uint);
			if (buffer == null || buffer.stride != stride || buffer.count != singleInstanceRenderArgs.Length || !buffer.IsValid())
			{
				buffer = new GraphicsBuffer(GraphicsBuffer.Target.IndirectArguments, singleInstanceRenderArgs.Length, stride);
			}

			buffer.SetData(singleInstanceRenderArgs);
			GraphicsBuffer.CopyCount(appendBuffer, buffer, dstOffsetBytes: 0);
		}
		
		public static void CreateArgsBuffer(ref GraphicsBuffer argsBuffer, Mesh mesh, int numInstances)
		{
			const int stride = sizeof(uint);
			const int numArgs = 5;
			const int subMeshIndex = 0;
			...
			{
				Release(argsBuffer);
				argsBuffer = new GraphicsBuffer(GraphicsBuffer.Target.IndirectArguments, numArgs, stride);
			}
		...
		public static GraphicsBuffer CreateArgsBuffer(Mesh mesh, GraphicsBuffer appendBuffer)
		{
			...
			GraphicsBuffer.CopyCount(appendBuffer, buffer, sizeof(uint));
			return buffer;
		}

		// Read number of elements in append buffer
		public static int ReadAppendBufferLength(GraphicsBuffer appendBuffer)
		{
			GraphicsBuffer countBuffer = new GraphicsBuffer(GraphicsBuffer.Target.Raw, 1, sizeof(int));
			GraphicsBuffer.CopyCount(appendBuffer, countBuffer, 0);
		...
		public static T[] ReadbackData<T>(GraphicsBuffer buffer)
		{
